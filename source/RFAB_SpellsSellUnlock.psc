Scriptname RFAB_SpellsSellUnlock extends ReferenceAlias
...
Function spellPerkCheck()
    if Player.HasPerk(AlterationAdept)
        PCAlterationAdept.SetValue(0 as Float)
    endif
    if Player.HasPerk(AlterationExpert)
        PCAlterationExpert.SetValue(0 as Float)
    endif
    if Player.HasPerk(AlterationMaster)
        PCAlterationMaster.SetValue(0 as Float)
    endif
    if Player.HasPerk(DestructionAdept)
        PCDestructionAdept.SetValue(0 as Float)
    endif
    if Player.HasPerk(DestructionExpert)
        PCDestructionExpert.SetValue(0 as Float)
    endif
    if Player.HasPerk(DestructionMaster)
        PCDestructionMaster.SetValue(0 as Float)
    endif
    if Player.HasPerk(IllusionAdept)
        PCIllusionAdept.SetValue(0 as Float)
    endif
    if Player.HasPerk(IllusionExpert)
        PCIllusionExpert.SetValue(0 as Float)
    endif
    if Player.HasPerk(IllusionMaster)
        PCIllusionMaster.SetValue(0 as Float)
    endif
    if Player.HasPerk(RestorationAdept)
        PCRestorationAdept.SetValue(0 as Float)
    endif
    if Player.HasPerk(RestorationExpert)
        PCRestorationExpert.SetValue(0 as Float)
    endif
    if Player.HasPerk(RestorationMaster)
        PCRestorationMaster.SetValue(0 as Float)
    endif
    if Player.HasPerk(ConjurationAdept)
        PCConjurationAdept.SetValue(0 as Float)
    endif
    if Player.HasPerk(ConjurationExpert)
        PCConjurationExpert.SetValue(0 as Float)
    endif
    if Player.HasPerk(ConjurationMaster)
        PCConjurationMaster.SetValue(0 as Float)
    endif
