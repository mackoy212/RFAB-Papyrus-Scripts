...
Function PollingInit()
	...
	;Цикл для удаления из массива SXPNGameStat нулевых значений, переопределяется после каждого изменения
	;в МСМ меню раздела Разное(Misc) (см zzzsxpconfig.psc строка "XPoll.SetPollingInterval(SXPGeneral[3]) ;запуск пересчета массива SXPNGameStat)"
	int i = 0
	int z1 = 0
	int ii = SXPQueryGameStat.Length
	ExcludeZero_SXPNGameStat = Utility.CreateIntArray(ii)
	while i < ii
		if SXPGameStats[i] != 0
			z1 += 1
			ExcludeZero_SXPNGameStat[z1 - 1] = i
		endif
		i += 1
	endwhile	
	SXPNGameStat = Utility.CreateIntArray(z1)	
	int iIndex = 0
	int iElement = SXPNGameStat.Length
	While iIndex < iElement
		int z2 = ExcludeZero_SXPNGameStat[iIndex]
		SXPNGameStat[iIndex] = Game.QueryStat(SXPQueryGameStat[z2])
		iIndex += 1
	EndWhile
EndFunction
...
Function PollGameStat(int iIndex)
	int z2 = 0
	z2 = ExcludeZero_SXPNGameStat[iIndex]
	int n_old = SXPNGameStat[iIndex]
	int n_new = Game.QueryStat(SXPQueryGameStat[z2])
		if ( n_new > n_old )			
			int n_sub = n_new - n_old
			float n_xp = SXPGameStats[z2] * n_sub		
			SXPNGameStat[iIndex] = n_new
			XPC.GainXPMessage(z2, n_xp)
		endif		
EndFunction
