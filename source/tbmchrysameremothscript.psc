Scriptname tbmChrysamereMothScript extends tbmCritterBase
{Script attached to Chrysamere moths.}
...
float property fRadius = 500.0 auto
{Maximum player flock distance}
Weapon property IngredientType auto
...
; ; 	Debug.TraceConditional("Moth " + self + " warping to state AtPlant", bCritterDebug)
	...
	if !(Game.GetPlayer().GetDistance(self) <= fMaxPlayerDistance )
		; Kill this critter
		debug.trace("player is too far away!")
		DisableAndDelete()
	else
		WarpToNewPlant()
...
State AtPlant
	...
	Event OnUpdate()
		...
		if !(Game.GetPlayer().GetDistance(self) <= fMaxPlayerDistance )
			; Kill this critter
			debug.trace("player is too far away (at plant)")
			DisableAndDelete()
...
; 		traceConditional(self + " reached goal", bCritterDebug)
		if !(Game.GetPlayer().GetDistance(self) <= fMaxPlayerDistance )
			...
			debug.trace("player too far away (goal reached)")
			DisableAndDelete()
		else
