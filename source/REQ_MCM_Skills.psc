...
function OnPageReset(String page)
	...
	elseIf page == "$REQ_cat_skill0"
		...
		self.AddTextOptionST("Persistence", "устойчивость", (Player.getAV("HeavyArmorSkillAdvance") as Int) as String + "%", 0)
	...
	elseIf page == "$REQ_cat_skill1"
		...
		index = data.derived.find("DestructionPowerMod", 0)
		self.AddTextOptionST("DestructionPowerMod_bonus", "сила и длительность заклинаний", "+" + (data.modifiers[index] as Int) as String + "%", 0)
		self.AddHeaderOption("$REQ_skillheader_3", 0)
		self.AddTextOptionST("WordsLearned", "$REQ_WordsLearned", "+" + ((-data.Mods_Shout[0] / 1.50000 * 100 as Float) as Int) as String + "%", 0)
		self.AddTextOptionST("WordsUnlocked", "$REQ_WordsUnlocked", "+" + ((-data.Mods_Shout[1] / 1.50000 * 100 as Float) as Int) as String + "%", 0)
	...
	elseIf page == "$REQ_cat_skill_expertise"
		...
		self.AddTextOption("Бой без оружия", (Player.getAV("PickpocketMod") as Int) as String, 0)
		self.AddTextOption("Повышение одноручных силовых атак", (Player.getAV("OneHandedSkillAdvance") as Int) as String + "%", 0)
		self.AddTextOption("Повышение двуручных силовых атак", (Player.getAV("TwoHandedSkillAdvance") as Int) as String + "%", 0)
		self.AddTextOption("Повышение безоружных силовых атак", (Player.getAV("PickpocketSkillAdvance") as Int) as String + "%", 0)
	...
	elseIf page == "$REQ_cat_weapon_stats"
		...
		If Player.GetEquippedItemType(0) == 1 || \
			...
			If EqWeapL.HasKeyword(game.GetForm(1092122) as Keyword)
				...
				Material = ""
			EndIf
			If EqWeapL.HasKeyword(game.GetFormFromFile(0x015EE3, "RFAB_AE.esp") as Keyword)
				self.AddTextOption(Material, "Серебро", 0)
				Material = ""
			EndIf
			If EqWeapL.HasKeyword(game.GetFormFromFile(0x015EE4, "RFAB_AE.esp") as Keyword)
				self.AddTextOption(Material, "Серебро", 0)
				Material = ""
			EndIf
		...
		If Player.GetEquippedItemType(1) != 0 && \
			...
			If EqWeapR.HasKeyword(game.GetForm(1092122) as Keyword)
				...
				Material = ""
			EndIf
			If EqWeapR.HasKeyword(game.GetFormFromFile(0x015EE3, "RFAB_AE.esp") as Keyword)
				self.AddTextOption(Material, "Серебро", 0)
				Material = ""
			EndIf
			If EqWeapR.HasKeyword(game.GetFormFromFile(0x015EE4, "RFAB_AE.esp") as Keyword)
				self.AddTextOption(Material, "Серебро", 0)
				Material = ""
			EndIf
